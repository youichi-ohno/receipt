...
//新レシート表ver.3.1.4
//assembled on 2026/6/8    last update 2026/9/1
...
function makepdfButtunClick() {
    ...
    activeSheet.getRange(1, 3, 2, 12).merge().clearContent();
    let values = activeSheet.getRange('a1:a6').getValues();
    ...
    let check5 = typeof Boolean(values[5]);

    if (check0 == "[object Date]" && check1 == "[object Date]" && check2 == true && check3 == "boolean" && check4 == "string" && check5 == "boolean") {
...
function receiptSheetMain(argEnterValues) {
    let begindate, enddate, dateinvalid, print, center, orderinvalid, i = 0;
    let enterValues = [];
    argEnterValues.map(function (e) {
        [[begindate], [enddate], [print], [dateinvalid], [center], [orderinvalid]] = argEnterValues;

        if (enterValues[i] == "") {
            Logger.log("値が異常です");
        }
        [begindate, enddate, print, dateinvalid, center, orderinvalid].flat(1);
    });
    ...
    function eachValueProcess(begindate, enddate, print, dateinvalid, center, employees2) {
        ...
        //昇順にソート
        //*************************************** */
        let newWriteValues1 = descOrder11(employees, orderinvalid);

        let newWriteValues2 = groupChief1(newWriteValues1);

        let writeValuesLength = Math.ceil(employees.length / 9) * 9;
        let negativeAmount = writeValuesLength - employees.length;

        let newWriteValues3 = receiptSort1(newWriteValues2, negativeAmount);

        drawBody(newWriteValues3, statusvalues2, filesId, folders);
    }
...
function drawBody(writeValues = [], printValues = [], filesId = [], folders = []) {
    ...
    if (printValues[2] == 0) {  //印刷枚数が0のとき（デフォルト）
        ...
        merge(folders[1].getId(), folders[2].getId(), folders[3]);
    }
    ...
    function draw(writeText0 = [], writeText1 = [], writeText2 = [], printValues = [], fileId, print, date, fileId11 = [], filesId) {
        ...
        function write() {
            ...
            if (printValues[3] === Boolean(true)) {
                ...
                date12 = String(Utilities.formatDate(new Date(date), 'JST', 'yyyy/M/d'));
            }
...
//pdfファイル結合関数
async function merge(targetFolderId, exportFolderId, fileName) {
    //まとめて結合の対象フォルダ
    ...
    await MergeFiles(target, exportman, fileName);
    ...
    //PDFをまとめて結合する関数
    async function MergeFiles(target, exportman, fileName) {
        //指定のフォルダ内のPDFを検索
        ...
        //PDF結合を実行
        let myPDF = await mergeAllPDFs(blobs, fileName);
        //出力先フォルダに出力
        ...
        activeSheet.getRange(1, 3, 2, 12).merge().setFontColor("blue").setFontSize(18).setValue(pdfUrl);
...
function onOpen() {
    ...
    sheet.getRange('a1:a4').setValues([[formatteddate], [formatteddate], [""], [false], [true]]);
...
function descOrder11(val1 = [], orderSelect) {
    let result;
    if (Boolean(orderSelect)) {
        result = val1.sort(function (a, b) {
            return a[4] - b[4];
        });
        return result;
    } else {
...
//グループ長並び替え
function groupChief1(val1 = []) {
    let temp = [], newWriteValues = [];

    val1.filter(function (val2) {
        if (val2[2] !== "") {
            temp.push(val2);
        }
    });
    ...
    val1.filter(function (val2) {
        if (val2[2] === "") {
            newWriteValues.push(val2);
        }
    });

    return [...temp, ...newWriteValues];

}

//受付並び替え（受付最終要素）
function receiptSort1(val1, repeatNumber) {
    let newwritevalues0 = [], temp = [], blank = [];
    ...
    return [...newwritevalues0, ...blank, ...temp];
}
